...
const DebitPage = () => {
    ...
    const fetchDebitEntries = async () => {
        try {
            const response = await fetch('http://localhost:5500/api/v1/getexpense'); 
            if (!response.ok) throw new Error('Network response was not ok');
            const data = await response.json();
            setDebitEntries(data.expenseEntries);
        } catch (error) {
    ...
    const handleDelete = async(id) => {
        ...
        try {
            ...
            setDebitEntries((prevEntries) => prevEntries.filter((entry) => entry._id !== id));
